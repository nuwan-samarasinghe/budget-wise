...
export const fetchYearlyExpense = createAsyncThunk(
  'expense/fetchYearly',
  async () => {
    await new Promise((res) => setTimeout(res, 500));
    //TODO waitting for backend
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMonthlyExpense.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.error.message || 'Failed to fetch monthly expense details';
      })
      ...
      .addCase(fetchYearlyExpense.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.error.message || 'Failed to fetch yearly expense details';
      })
      ...
      .addCase(insertExpense.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.error.message || 'Failed to insert expense details';
      });
  },
});
