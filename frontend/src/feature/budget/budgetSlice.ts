...
export const insertBudget = createAsyncThunk('budget/insert', async (budget: Budget) => {
    await new Promise((res) => setTimeout(res, 500));
...
export const fetchMonthlyBudget = createAsyncThunk('budget/fetchMonthly', async () => {
    await new Promise((res) => setTimeout(res, 500));
    //TODO waitting for backend
    return [
...
export const fetchYearlyBudget = createAsyncThunk('budget/fetchYearly', async () => {
    await new Promise((res) => setTimeout(res, 500));
    //TODO waitting for backend
    return [
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchMonthlyBudget.rejected, (state, action) => {
                state.loading = false;
                state.error = action.error.message || 'Failed to fetch monthly budget details';
            })
            ...
            .addCase(fetchYearlyBudget.rejected, (state, action) => {
                state.loading = false;
                state.error = action.error.message || 'Failed to fetch yearly budget details';
            })
            .addCase(insertBudget.pending, (state) => {
                state.loading = true;
                state.error = null;
            })
            .addCase(insertBudget.fulfilled, (state, action: PayloadAction<Budget>) => {
                state.loading = false;
                const index = state.budgets.findIndex(i => i.id === action.payload.id);
                if (index !== -1) {
